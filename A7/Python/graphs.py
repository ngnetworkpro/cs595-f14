...
def buildClubGraphs(inFile, n):
  ...
  # Create the json file for the whole karate club
  d = json_graph.node_link_data(K)
  json.dump(d, open('kc.json','w'))
  ...
  # write the json files for the split clubs
  dd = json_graph.node_link_data(K) # node-link format to serialize
  # write json
  json.dump(dd, open('split.json','w'))
